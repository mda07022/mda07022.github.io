...
function get_parent_id_with_array(opts,arr) {
	...
	if(ztree_toc_check_pId(result, opts) || result == 0){
		return result;
	}else{
		return get_parent_id_with_array(opts, result_arr);
	}
}
 ...
 * @param {Object} opts
 */
function ztree_toc_check_pId(pid, opts){
	for(var i = 0; i < opts._header_nodes.length; i++){
		if(pid == opts._header_nodes[i].id){
			return true;
		}
	}
	return false;
...
;(function($) {
	...
	function _add_header_node(opts ,header_obj) {
		...
		var anchor = "toc_" + id;//use_head_anchor.html#第二部分
